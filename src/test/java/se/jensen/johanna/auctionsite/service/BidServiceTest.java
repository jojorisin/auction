...
class BidServiceTest extends AuctionTestBase {
  ...
  void placeNormalBidUnderAcceptedLeading() {
    when(auctionRepository.findByIdForBidding(any())).thenReturn(Optional.of(auction));
    when(userRepository.findById(any())).thenReturn(Optional.of(currentBidder));

    BidResponse response = bidService.placeBid(new BidRequest(normalBidAmount), BIDDER_ID,
        AUCTION_ID);
  ...
  void placeMaxBidOverAcceptedLeading() {
    ...
    BidResponse response = bidService.placeBid(new BidRequest(overAcceptedAmount), BIDDER_ID,
        AUCTION_ID);
  ...
  void shouldPlaceMaxBidAndReturnOutbid() {
    ...
    BidResponse response = bidService.placeBid(new BidRequest(overAcceptedAmount * 2), BIDDER_ID,
        AUCTION_ID);
  ...
  void shouldRaiseWithMaxBidForLeadingBidder() {
    ...
    BidResponse response = bidService.placeBid(new BidRequest(acceptedAmount), BIDDER_ID,
        AUCTION_ID);

    when(auctionRepository.findByIdForBidding(any())).thenReturn(Optional.of(auction));
    when(userRepository.findById(any())).thenReturn(Optional.of(currentBidder));

    BidResponse raisedResponse = bidService.placeBid(new BidRequest(acceptedAmount + 1), BIDDER_ID,
        AUCTION_ID);
